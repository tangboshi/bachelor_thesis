...
\begin{figure}[ht]
	...
	% source: http://www.electroschematics.com/9043/am-receiver-circuit/ 03.10.17
	\begin{center}
		\subfloat[GNU Radio Logo]{\includesvg[width=0.4\textwidth]{pictures/gnuradio_logo}}
		\qquad
		\subfloat[GRC GUI]{\includegraphics[width=0.5\textwidth,valign=c]{pictures/grc_ui}}
	\end{center}
	\caption{GNU Radio \& GNU Radio Companion}
\end{figure}
...
The two most basic concepts of GNU Radio are flowcharts and blocks. As mentioned in \ref{sec:gnu-radio} flowcharts are directed graphs, whose nodes are functional blocks and whose vertices determine the direction of data flow. 
...
Polymorphic types (PMT) are opaque data types that enable safe information exchange across blocks by serving as generic container of data. Self-evidently, the original data type must be retained as a PMT class member. For thread-safety reasons PMT are immutable. We make extensive use of PMTs when passing messages. As an aside, note that the python PMT class has some powerful tools unavailable its C++ counterpart, making use of Python's weak typing.
